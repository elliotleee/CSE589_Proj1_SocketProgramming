	...
	sort(socketlist.begin(), socketlist.end());
  	for (unsigned int i = 0; i < socketlist.size(); ++i) {
    	cse4589_print_and_log("%-5d%-35s%-20s%-8s\n", i + 1,
                          ...
                          socketlist[i].ip.c_str(), socketlist[i].port.c_str());
  }
  cse4589_print_and_log("[%s:END]\n", cmd.c_str());
}
...
// 		for(int i = 0; i < 4; ++i){
// 		  if(res[i][0] == "") break;
// 		  cse4589_print_and_log("%-5d%-35s%-20s%-8d\n", i + 1, res[i][0].c_str(), res[i][1].c_str(), str_to_int(res[i][2]));
// 		}
// 		cse4589_print_and_log("[%s:END]\n", command);
// }
...
bool checkIfIPBlocked(SocketObject* hd, std::string ip) {
...
int server_process(string MYPORT) {
   ...
   // cout << "start SUCCESS";
	...
	while(1){
    	...
    	for (int i = 0; i < 4; i++) { // add child sockets to set
      		int sd = masterlist[i];
		...
		if (FD_ISSET(fileno(stdin), &read_fds)) {
 	     	...
 	     	switch (mark){
				...
				case 2:{//*********************************Server statics******************************************
					log_STATICS();
					break;     
				} 
				case 6:{//*********************************Server blocked******************************************
      ...
      //handleCommands(msg, 1);
    		...
    		int fdtemp= accept(serversocketfd,(struct sockaddr *)&remoteaddr,&addrlen);
    		for(int i = 0; i<4; i++){
    			if (masterlist[i] == 0 ){
    				masterlist[i] = fdtemp;
    				break;
    		...
    		msg = charmsg;
							        ...
							        case 9:{ //*********************************Message info******************************************
										...
										if(hd == NULL){
											...
											if(ret == hd->blockeduser.end()){
												...
												send(hd->cfd, (const char*)msg.c_str(), sizeof msg, 0);
							        ...
							        case 1:{ //*********************************Message hostname******************************************
										...
										if(hd == NULL) {
											...
											if(!hd->msgbuffer.empty()){
												for(vector<string>::iterator it= hd->msgbuffer.begin(); it < hd->msgbuffer.end(); it++){
													string mgs = *it;
													send(hd->cfd, (const char*)mgs.c_str(),sizeof mgs,0);
												}
												hd->msgbuffer.clear();
											}
										}
										...
										for (unsigned int i = 0; i < socketlist.size(); ++i) {
									    	if (socketlist[i].status == "logged-in") {
									      		message += socketlist[i].hostname + " " + socketlist[i].ip + " " +
									             socketlist[i].port;
									    }
									  }
							  ...
							  //       	string to_ip = msg_p[1];
									...
									// 						if(socketlist[i]->status == "logged-in"){
									// 						send(socketlist[i]->cfd, msg, sizeof msg, 0);
			    ...
			    for(int p = 0; p<= 4; p++){
			    	int fdtemp =  masterlist[p];
			    	if (fdtemp == 0)break;
			    	if(FD_ISSET(fdtemp,&master))
			    		memset(charmsg,0,sizeof charmsg);

		     		 	 if(recv(fdtemp, charmsg, sizeof charmsg, 0) == 0){
		     				close(fdtemp);  
		     				FD_CLR(fdtemp,&master);
		     				for(int i = 0; i<4; i++){
				    			if (masterlist[i] == fdtemp ){
				    				masterlist[i] = 0;
				    				break;
				    			}
				    		}       			   
